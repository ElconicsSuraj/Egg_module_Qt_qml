...
class GPIO:
    ...
    @staticmethod
    def setmode(mode):
        GPIO._handle = lgpio.gpiochip_open(0)

    @staticmethod
    def setup(pin, mode):
        if mode == GPIO.OUT:
            lgpio.gpio_claim_output(GPIO._handle, pin)
        else:
            lgpio.gpio_claim_input(GPIO._handle, pin)

    @staticmethod
    def output(pin, value):
        lgpio.gpio_write(GPIO._handle, pin, int(value))

    @staticmethod
    def input(pin):
        return lgpio.gpio_read(GPIO._handle, pin)

    @staticmethod
    def cleanup():
        if GPIO._handle:
            lgpio.gpiochip_close(GPIO._handle)
